...
import { Sun, MessageSquare, Send, AlertTriangle, ThumbsUp, Users } from 'lucide-react';
...
function DayPhase({ 
  ...
  const chatMessages = gameState?.gameLog?.filter(log => log.type === 'chat') || [];
    ...
    return (
      <div className="bg-gray-900/50 backdrop-blur-md rounded-lg p-8 border border-gray-600 text-center">
        ...
        <div className="mt-6 bg-gray-800/50 rounded-lg p-4 max-h-64 overflow-y-auto">
          <h4 className="text-gray-400 font-semibold mb-3 text-sm">Village Discussion</h4>
            ...
            <div className="space-y-2">
              {chatMessages.slice(-10).map((log, index) => (
                <div key={index} className="bg-gray-700/50 rounded p-2">
                  <p className="text-gray-300 font-semibold text-xs">{log.player}</p>
    ...
    <div className="space-y-6">
      ...
      <div className="bg-gradient-to-r from-yellow-600 to-orange-600 rounded-lg p-6 border-2 border-yellow-400 shadow-2xl">
        ...
        <p className="text-yellow-100">
          Discuss with other villagers to find the werewolves. Accuse suspicious players and vote them out!
        </p>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-orange-200">
                  <span className="font-semibold">{lastAccusation.accuser}</span> accused{' '}
                  <span className="font-semibold text-red-300">{lastAccusation.target}</span>
                </p>
              </div>
            </div>
            ...
            {lastAccusation.accuserId !== myPlayer?.id && (
              <button
                ...
                className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg font-semibold transition-all transform hover:scale-105 flex items-center space-x-2"
              >
                <ThumbsUp className="w-4 h-4" />
                <span>Second & Vote</span>
              </button>
            )}
              ...
              <div className="space-y-3">
                {chatMessages.slice(-20).map((log, index) => {
                  ...
                  return (
                    ...
                    >
                      <p className={`font-semibold text-xs ${isMe ? 'text-purple-200' : 'text-blue-200'}`}>
                        {log.player} {isMe && '(You)'}
                      </p>
        ...
        <div className="bg-white/10 backdrop-blur-md rounded-lg border border-white/20">
          ...
          <div className="p-4">
            <p className="text-purple-200 text-sm mb-4">
              Select a player you suspect is a werewolf. If another player seconds your accusation, a vote will begin.
            </p>
            ...
            <div className="space-y-2 max-h-96 overflow-y-auto custom-scrollbar">
              {alivePlayers.length === 0 ? (
                ...
                alivePlayers.map((player) => {
                  const hasBeenAccused = accusations.some(acc => acc.targetId === player.id);
                  const lastAccusedByMe = accusations
                    .slice()
                    .reverse()
                    .find(acc => acc.accuserId === myPlayer?.id);
                  const iAccusedThisPlayer = lastAccusedByMe?.targetId === player.id;
                      ...
                      <div className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
                          ...
                          <div>
                            ...
                            {hasBeenAccused && (
                              <span className="text-red-400 text-xs block">⚠️ Accused</span>
                            )}
                          </div>
                        </div>
          ...
          <div className="px-4 pb-4">
            <div className="bg-blue-900/30 border border-blue-500/50 rounded-lg p-3">
              <h5 className="text-blue-200 font-semibold text-xs mb-1">💡 How Accusations Work:</h5>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
          <div className="bg-white/10 rounded p-3">
            <p className="text-purple-200">
              <strong className="text-white">🔍 Observe:</strong> Watch who stays quiet and who's too aggressive
            </p>
          </div>
          <div className="bg-white/10 rounded p-3">
            <p className="text-purple-200">
              <strong className="text-white">🤔 Question:</strong> Ask players about their suspicions and reasoning
            </p>
          </div>
          <div className="bg-white/10 rounded p-3">
            <p className="text-purple-200">
              <strong className="text-white">🎭 Deduce:</strong> Use logic to find inconsistencies in stories
            </p>
          </div>
        </div>
      </div>
