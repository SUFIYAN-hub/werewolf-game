...
if (myRole === 'witch') {
  return <WitchNightPhase gameState={gameState} onNightAction={onNightAction} />;
}

// ✅ NEW: Detective gets special component
if (myRole === 'detective') {
  return <DetectiveNightPhase gameState={gameState} onNightAction={onNightAction} />;
}
  ...
  return (
    <div className="bg-orange-900/30 backdrop-blur-md rounded-lg p-8 border border-orange-500/50 text-center">
      ...
      <p className="text-orange-200 mb-4">
        As the Hunter, you have no night action. Rest well and prepare for the day ahead.
      </p>
      <div className="bg-orange-800/30 rounded-lg p-4 border border-orange-600/50">
        <p className="text-orange-300 text-sm">
          🎯 <strong>Your Power:</strong> When you are eliminated (by werewolves or voting), 
          you will immediately choose one player to take with you as your dying shot.
        </p>
      </div>
    </div>
  );
    ...
    {myRole === 'werewolf' && gameState?.nightInfo?.werewolfTeam && (
      <div className="mb-6 bg-red-900/30 border-2 border-red-500 rounded-lg p-4">
        <div className="flex items-center space-x-3 mb-3">
          ...
          <h4 className="text-white font-bold text-lg">Your Werewolf Pack 🐺</h4>
        </div>
        ...
        {gameState.nightInfo.werewolfTarget && (
          <div className="mt-3 bg-red-950/50 rounded-lg p-3 border border-red-600/50">
            <p className="text-red-200 text-sm">
              🎯 <strong>Pack Target:</strong> {gameState.nightInfo.werewolfTarget}
            </p>
        ...
        <div className="mt-3 bg-red-950/50 rounded-lg p-3 border border-red-600/50">
          <p className="text-red-200 text-xs">
            💡 <strong>Coordinate silently:</strong> All werewolves see the same information. Choose your target wisely!
          </p>
        </div>
      </div>
    )}
