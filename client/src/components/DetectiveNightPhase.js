  ...
  if (hasUsedAbility && !detectiveResult) {
    return (
      <div className="bg-indigo-900/30 border-2 border-indigo-500 rounded-lg p-6 text-center">
        ...
        <h3 className="text-2xl font-bold text-indigo-400 mb-2">Ability Already Used</h3>
      ...
      <div className="bg-indigo-900/30 border-2 border-indigo-500 rounded-lg p-6">
        ...
        <div className="bg-yellow-900/30 border-2 border-yellow-500 rounded-lg p-4">
          <p className="text-yellow-200 text-lg mb-3">
            <strong>{detectiveResult.player1Name}</strong> and <strong>{detectiveResult.player2Name}</strong>
          </p>
      ...
      <div className="bg-indigo-900/30 border-2 border-indigo-500 rounded-lg p-6">
        <div className="flex items-center space-x-3">
          ...
          <div>
            ...
            <p className="text-indigo-200">Analyzing the connection... Wait for results.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="bg-gradient-to-br from-indigo-600 to-indigo-800 rounded-lg p-6 border-2 border-indigo-400 shadow-2xl">
      ...
      <div className="mb-6">
        <div className="flex items-center space-x-3 mb-3">
          ...
          <div>
            <h3 className="text-2xl font-bold text-white">Detective Investigation</h3>
            <p className="text-indigo-200 text-sm">One-time use - Choose wisely!</p>
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <p className="text-white text-sm">
          Select two players to investigate. You'll learn if they're on the same team or different teams.
        </p>
      </div>
          ...
          {alivePlayers.map((player) => (
            ...
            >
              <div className="flex items-center space-x-3">
                ...
                {player1 === player.id && <Check className="w-5 h-5 text-white ml-auto" />}
              </div>
            </button>
          ))}
        </div>
            ...
            {alivePlayers.map((player) => (
              ...
              >
                <div className="flex items-center space-x-3">
                  ...
                  <span className="text-white font-semibold">{player.name}</span>
                  {player2 === player.id && <Check className="w-5 h-5 text-white ml-auto" />}
                </div>
              </button>
            ))}
          </div>
      ...
      {player1 && player2 && (
        <button
          ...
          className="w-full bg-white hover:bg-gray-100 text-indigo-900 font-bold py-4 px-6 rounded-lg shadow-lg transition-all transform hover:scale-105 mt-4"
        >
      ...
      <div className="mt-6 bg-indigo-950/50 rounded-lg p-4 border border-indigo-600/50">
        <h4 className="text-indigo-200 font-semibold text-sm mb-2">💡 Strategy:</h4>
