      ...
      <div className="space-y-2 mb-4">
        {alivePlayers.map((player) => {
          ...
          return (
            <div
              key={player.id}
              className={`${status.bgColor} border ${status.borderColor} rounded-lg p-3 transition-all hover:scale-102`}
            >
              <div className="flex items-center justify-between">
                ...
                {player.role && (player.id === myPlayerId || !player.isAlive) && (
      ...
      <div className="mt-4 pt-4 border-t border-white/20">
        <div className="grid grid-cols-2 gap-3 text-center">
          <div className="bg-green-900/30 rounded-lg p-2 border border-green-500/30">
            <p className="text-2xl font-bold text-green-400">{alivePlayers.length}</p>
            <p className="text-xs text-green-200">Alive</p>
          </div>
          <div className="bg-red-900/30 rounded-lg p-2 border border-red-500/30">
            <p className="text-2xl font-bold text-red-400">{deadPlayers.length}</p>
