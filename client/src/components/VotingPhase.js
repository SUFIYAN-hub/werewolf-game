  ...
  if (!votingTarget) {
    return (
      <div className="bg-white/10 backdrop-blur-md rounded-lg p-8 border border-white/20 text-center">
        ...
        <p className="text-purple-200">Waiting for someone to be accused...</p>
      </div>
    );
  }
    ...
    return (
      <div className="bg-gray-900/50 backdrop-blur-md rounded-lg p-8 border border-gray-600">
        <div className="text-center mb-6">
          ...
          <p className="text-gray-500">Watch the voting unfold...</p>
        </div>
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-red-400">
                  {alivePlayers.length > 0
                    ? Math.round((yesVotes / alivePlayers.length) * 100)
                    : 0}
                  %
                </span>
              </div>
              <div className="w-full bg-gray-700 rounded-full h-4">
                <div
                  ...
                  style={{
                    width: `${
                      alivePlayers.length > 0
                        ? (yesVotes / alivePlayers.length) * 100
                        : 0
                    }%`,
                  }}
                />
            ...
            <div>
              <div className="flex justify-between text-sm mb-1">
                ...
                <span className="text-green-400">
                  {alivePlayers.length > 0
                    ? Math.round((noVotes / alivePlayers.length) * 100)
                    : 0}
                  %
                </span>
              </div>
              <div className="w-full bg-gray-700 rounded-full h-4">
                <div
                  ...
                  style={{
                    width: `${
                      alivePlayers.length > 0
                        ? (noVotes / alivePlayers.length) * 100
                        : 0
                    }%`,
                  }}
                />
    ...
    <div className="bg-gradient-to-br from-red-700 to-pink-700 rounded-lg p-8 border-2 border-red-400 shadow-2xl">
      ...
      <div className="text-center mb-8">
        ...
        <p className="text-red-100 text-lg">
          Cast your vote on:{" "}
          <span className="font-bold text-white">{votingTarget.name}</span>
        </p>
      </div>
        ...
        <div className="grid grid-cols-2 gap-6">
          ...
          >
            <div className="flex flex-col items-center">
              <ThumbsDown className="w-12 h-12 mb-3" />
          ...
          >
            <div className="flex flex-col items-center">
              <ThumbsUp className="w-12 h-12 mb-3" />
              <h3 className="text-2xl font-bold mb-2">INNOCENT</h3>
              <p className="text-green-100 text-sm">Save {votingTarget.name}</p>
            </div>
          </Button>
        </div>
      )}
      ...
      <div className="mt-6 bg-yellow-900/30 border border-yellow-500/50 rounded-lg p-4">
        <p className="text-yellow-200 text-sm text-center">
          ⏱️ <strong>{votingTarget.name}</strong> has had 30 seconds to defend
          themselves
        </p>
      </div>
    </div>
