...
import React, { useState, useEffect } from 'react';
import { Clock, X, MapPin, Settings as SettingsIcon } from 'lucide-react';
...
function PrayerNotification({ location, onLocationChange }) {
  const [prayerTimes, setPrayerTimes] = useState(null);
  const [nextPrayer, setNextPrayer] = useState(null);
  const [timeUntilPrayer, setTimeUntilPrayer] = useState('');
  const [showNotification, setShowNotification] = useState(false);
  ...
  const [currentSettings, setCurrentSettings] = useState(getSavedPrayerSettings());
  ...
  return (
    <>
      ...
      {/* Prayer Time Indicator (Always visible) - Responsive */}
      <div 
        className="fixed top-2 left-2 sm:top-4 sm:left-4 bg-green-900/90 backdrop-blur-md text-white rounded-lg shadow-lg border border-green-500/50 z-50"
      >
        <div className="flex items-center">
          {/* Main Prayer Info (Clickable to show all times) */}
          <button
            onClick={() => setShowAllTimes(!showAllTimes)}
            className="px-2 py-1 sm:px-4 sm:py-2 hover:bg-green-800/90 transition-all rounded-l-lg"
          >
            <div className="flex items-center space-x-1 sm:space-x-2">
              <Clock className="w-3 h-3 sm:w-4 sm:h-4" />
              <div className="text-xs">
          ...
          <button
            ...
            className="px-2 py-3 sm:px-3 border-l border-green-500/50 hover:bg-green-800/90 transition-all rounded-r-lg"
      ...
      {showAllTimes && (
        <div className="fixed top-16 sm:top-20 left-2 sm:left-4 bg-green-900/95 backdrop-blur-md text-white p-4 rounded-lg shadow-2xl border border-green-500/50 z-50 max-w-xs">
          ...
          <div className="text-xs text-green-200 border-t border-green-500/30 pt-2 space-y-1">
            <p>📍 {prayerTimes.coordinates.latitude.toFixed(2)}°, {prayerTimes.coordinates.longitude.toFixed(2)}°</p>
            <p>🕌 Method: {prayerTimes.method}</p>
            <p>📚 Madhab: {prayerTimes.madhab}</p>
            ...
            <button
              ...
              className="w-full mt-2 px-3 py-2 bg-green-600 hover:bg-green-700 rounded-lg text-white text-xs font-semibold transition flex items-center justify-center space-x-2"
            >
